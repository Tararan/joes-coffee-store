      ...
      <div className="col-md-4">
        <article id={id} className={`js-target-id products__box 
          ...
          <div className="products__box-img-wrapper"
            onClick={(e) => { toggleOverlay(); getClickedProduct(e); }}
          >
            <img className="products__box-img"
              src={boximage} alt="product" />
          </div>
          <h2 className="products__box-title">
            {boxtitle}
          </h2>
          <p className="products__box-price">
            <span className="products__box-price-regular">{boxprice}</span>
            <span className="products__box-price-discount">{discountprice}</span>
          </p>
          <div className='button__wrapper'>
            <button className={`button 
              ${available ? 'button--product-box' : 'button--unavailable'}`}
              onClick={addToCart}>
